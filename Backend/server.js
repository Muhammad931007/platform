    ...
    if (!isKnownAction) {
      if (pathname.startsWith('/admin/') && !hasStaticFile) {
        if (/\/users\/add_users\.html$/.test(pathname) && body.username) {
          ...
          const newUser = { id: nextId, username: body.username, password: body.password || 'password123', paypwd: body.paypwd || '123456', tel: body.tel || '', email: body.email || '', invitecode: body.invitecode || ('ERP' + nextId), token: 'token_' + Date.now(), expiretime: 1999999999, balance: String(body.balance || '0.00'), freeze_balance: '0.00', today_income: '0.00', total_income: '0.00', vip_level: 1, credit: 100, status: '1', online: false, created_at: new Date().toISOString() };
