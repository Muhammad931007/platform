...
function localMemberRow(user) {
  ...
  const action = (label, url, kind = 'modal') => { const color = /禁用|禁止提现|重置佣金|配置奖励金/.test(label) ? 'layui-btn-danger' : /钱包|重置任务|普通方式|账变信息|连单列表|改单数|登录历史/.test(label) ? 'layui-btn-normal' : 'layui-btn-normal'; return `<a href="javascript:void(0)" data-modal="${url}" data-title="${label}" class="layui-btn layui-btn-sm ${color} local-action">${label}</a>`; };
